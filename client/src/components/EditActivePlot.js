...
import { Box, Typography, Grid, FormControl, FormLabel, RadioGroup, Radio, FormControlLabel } from '@mui/material';
...
export default function EditActivePlot(props) {
    ...
    const [headstone, setHeadstone] = useState((props.data && props.data.headstone) || '');
    const [reserved, setReserved] = useState((props.data && props.data.reserved) || '');
    const [comments, setComments] = useState((props.data && props.data.comments) || '');
    const [plotFilledStatus, setPlotFilledStatus] = useState(props.data && props.data.plotFilledStatus);
    ...
    function sendUpdatedData(updatedFields) {
        axios.post(`/plots/edit/${props.data.airtableId}`, updatedFields)
            .then(response => {
                if(response.data.statusCode > 200) {
                    ...
                    if(!updateResults.hasOwnProperty('comments')) updateResults.comments = ''
                    if(!updateResults.hasOwnProperty('reserved')) updateResults.reserved = ''
                    if(!updateResults.hasOwnProperty('headstone')) updateResults.headstone = ''
    ...
    return (
        ...
        <>
            <Box flexGrow={1} sx={{ backgroundColor: 'rgb(19, 33, 69)', color: 'white', borderRadius: '12px', padding: '20px', minHeight:'220px' }}>
                <Typography mb={3} variant='h5' textAlign='center' fontWeight={300} color='rgb(144, 154, 176)'>Edit Plot</Typography>
                <Grid container spacing={3} mb={3}>
                    ...
                    <Grid item xs={12} md={6}>
                        <Box sx={{display: {xs: 'flex', lg:'inherit'}, justifyContent: {xs: 'center'}}}>
                            <FormControl component="fieldset">
                                ...
                                <RadioGroup row aria-label="plot status" name="plotFilledStatus" onChange={handleChange} value={plotFilledStatus}
                                    sx={{
                                        '& .MuiRadio-root':
                                        {
                                            color: 'rgb(144, 154, 176)',
                                        ...
                                        <FormControlLabel value="available" labelPlacement='top' control={<Radio />} label="Available" />
                                        <FormControlLabel value="reserved" labelPlacement='top' control={<Radio />} label="Reserved" />
                                        <FormControlLabel value="interred" labelPlacement='top' control={<Radio />} label="Interred" />
                                    </RadioGroup>
                            </FormControl>
                        </Box>
                    </Grid>
                </Grid>
                <Box sx={{ display: { xs: 'flex', sm: 'inherit' }, justifyContent: { xs: 'center' } }}>
                    <StyledButton variant='contained' color="error" onClick={handleClick} sx={{ marginRight: '15px' }}>
                        Cancel Edit
                    </StyledButton>
                    <StyledButton variant='contained' onClick={handleSubmit} disabled={isButtonDisabled}>
                        Submit
                    </StyledButton>
                </Box>
            </Box>
